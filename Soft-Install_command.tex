...
%Introduction
\textrm{CP2k}脚本安装软件有极大的方便，其脚本的位置在
\textrm{\backslash\$CP2k\_install\_dir\backslash tools\backslash toolchai\backslash scripts\backslash stage[i]\backslash XXX.sh}\\
安装过程中由以\textrm{gcc}安装需要注意，\textrm{configure}中需要设置
\textrm{unset LIBRARY\_PATH CPATH C\_INCLUDE\_PATH PKG\_CONFIG_PATH CPLUS\_INCLUDE\_PATH INCLUDE}
...
\section{\rm{systemctl}}
\textrm{systemctl}是\textrm{systemd}的命令行工具，主要用于控制\textrm{systemd}系统和服务管理器
重启指定服务:~使用\textrm{systemctl restart}服务名命令来重新启动一个服务。例如，要重启\textrm{Apache}服务，可以使用\textrm{systemctl restart httpd}。

查看服务状态:~使用\textrm{systemctl status}服务名来检查服务是否正常运型
...
\item 重启加载\textrm{Docker}\\
	\textrm{sudo~systemctl~daemon-reload}\\
