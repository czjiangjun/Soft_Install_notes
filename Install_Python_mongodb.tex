...
\fbox{
	\parbox{\textwidth}{
		\textrm{
			sudo apt install \textcolor{blue}{build-essential zlib1g-dev libncurses5-dev libgdbm-dev libnss3-dev libssl-dev libreadline-dev libffi-dev wget libbz2-dev liblzma-dev sqlite3 libsqlite3-dev tk-dev uuid-dev libgdbm-compat-dev}}}}
\vskip 5pt
\fbox{
	\parbox{\textwidth}{
		\textrm{
...
\fbox{
	\parbox{\textwidth}{
		\hspace*{25pt}\textrm{JOB\_STORE:}~$\ast$系统指定的存储数据名(\textrm{JOB\_STORE})，不可改动\\
		\hspace*{27pt}\textrm{docs\_store:}\\
		\hspace*{29pt}\textrm{type:~MongoStore}~$\ast$人工指定的存储计算对象的数据库名\\
		\hspace*{29pt}\textrm{database:~MongoDB\_FOR\_ATOMATE}\\
		\hspace*{29pt}\textrm{host:~localhost}\\
		\hspace*{29pt}\textrm{port:~27017}\\
		\hspace*{29pt}\textrm{username: }\\
		\hspace*{29pt}\textrm{password: }\\
		\hspace*{29pt}\textrm{collection\_name:~outputs}~$\ast$计算对象数据的\textrm{collection}名\\
		\hspace*{27pt}\textrm{additional\_stores:}\\
		\hspace*{29pt}\textrm{data:}\\
		\hspace*{31pt}\textrm{type:~GridFSStore}\\
		\hspace*{31pt}\textrm{database:~MgO}~$\ast$人工指定的存储计算流程与任务的数据库名\\
		\hspace*{31pt}\textrm{host:~localhost}\\
		\hspace*{31pt}\textrm{port:~27017}\\
		\hspace*{31pt}\textrm{username: }\\
		\hspace*{31pt}\textrm{password: }\\
		\hspace*{31pt}\textrm{collection\_name:~outputs\_blobs}~$\ast$计算流程与任务数据的\textrm{collection}名(可以是多个)
	}}\\
		该文件写入\textrm{.bashrc}，成为环境变量:~\textcolor{blue}{JOBFLOW\_CONFIG\_FILE}\\
		\textrm{export JOBFLOW\_CONFIG\_FILE=/home/jun-jiang/WORKS/ATOMATE2_TEST/Config/jobflow.yaml}
\end{itemize}

\section{\rm{BaxKB}的安装和使用}
\textrm{MaxKB~(Max Knowledge Base)}是一款基于大语言模型\texrm{LLM~(Large Language Model)}的知识库问答系统
...
\textrm{docker exec -it ollama ollama run gemma:7b} %\textrm{Gemma}是\textrm{Google~AI}的模型
